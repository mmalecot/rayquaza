...
pub const BUILDING_COUNT: i32 = WORLD_WIDTH / 100;
...
fn main() -> Result {
    let window = WindowBuilder::new()
        ...
        .msaa_4x()
        .build()?;
    let mut player = Rectangle::new(WORLD_WIDTH as f32 / 2.0, 280.0, 40.0, 40.0);
    ...
    for index in 0..BUILDING_COUNT {
        ...
        let rectangle = Rectangle::new(
            index as f32 * 100.0,
            window.get_height() as f32 - 130.0 - height as f32,
            100.0,
            height as f32,
        );
        let color = Color::new(
    ...
    while !window.should_close() {
        ...
        if window.is_mouse_button_down(MouseButton::Left) {
            camera.rotation += 100.0 * window.get_frame_time();
        }
        ...
        window.draw(|canvas| {
            ...
            canvas.draw_rectangle(10, 10, 250, 115, Color::BLACK.fade(0.5));
            canvas.draw_rectangle_lines(10, 10, 250, 115, Color::BLACK);
            canvas.draw_text("Free 2D camera controls:", 20, 20, 10, Color::WHITE);
            canvas.draw_text("- Right / Left to move offset", 40, 40, 10, Color::WHITE);
            ...
            canvas.draw_text("- R to reset zoom and rotation", 40, 100, 10, Color::WHITE);
        });
    }
    Ok(())
}
